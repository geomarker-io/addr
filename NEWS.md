...
- `nad()`, `nad_read()`, and `nad_download()` now default to National Address Database revision 23. Revision 22 remains supported for existing local source and derived files, but its removed USDOT source asset is no longer downloaded; forced revision 22 refreshes fail without deleting local archives or partial downloads.
