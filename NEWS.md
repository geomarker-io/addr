...
- Breaking: `nad()`, `nad_read()`, and `nad_download()` now support only revision 23. stow installs the pinned USDOT compressed flat-file archive as a durable managed local copy under `stow/nad`; a native streaming reader filters the requested county without unpacking the roughly 41 GB text member, and `nad()` stores the transformed county separately under `v1/nad/23` in addr's user data directory. Each atomic county install is inventoried with row count, size, SHA-256 digest, revision, and timestamp in `v1/nad_manifest/23/counties.parquet`; `nad_manifest()` reads or fully validates that inventory. Matching `addr-nad-fuel` pack and install scripts bundle both processed directories with a validated JSON sidecar, while excluding the stow-managed national source. Existing development-only county files under `stow/nad/v1` are not searched or migrated. The packaged `nad_example_data()` and prepared matching index have been rebuilt from Hamilton County revision 23 data.
